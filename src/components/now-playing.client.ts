...
export const initNowPlayingWidget = (root: HTMLElement) => {
	...
	const setCover = (url: string | null | undefined) => {
		...
		const currentSrc = cover.src || cover.getAttribute("src");
		const currentUrl = currentSrc ? new URL(currentSrc, location.href).href : "";
		const targetUrl = url.startsWith("http") ? url : new URL(url, location.href).href;
	...
	const handlePayload = (payload: Payload, fetchStartMs: number) => {
		...
		if (!payload?.track) {
			const errorMsg = typeof (payload as Record<string, unknown>).error === "string"
			? (payload as Record<string, unknown>).error as string
			: undefined;
			setIdleState(errorMsg || "Not listening right now");
			announce("idle:none", errorMsg || "Not listening right now.");
